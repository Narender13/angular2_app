...
@Component({
    selector: 'page-umbrella-policy',
    templateUrl: 'umbrellapolicy.html'
})
export class UmbrellaPolicyPage implements OnInit {
    ...
    ngOnInit() {
        this.umbrellaPolicyData = this.navParams.get('param1');
        this.index = this.navParams.get('param2');
        this.customerName = this.navParams.get('param3');
        this.umbrellaPolicyData.csppolicyNickName = "UMBRELLA";
    }

    policyDocumentPage() {
        this.navCtrl.push(PolicyDocumentPage, {
            param1: this.umbrellaPolicyData,
            param2: this.index,
      ...
      myBillsPage() {
        let text: string;
               this.paymentusService.getPolicy(this.umbrellaPolicyData)
                    .map((policyRequest) => {

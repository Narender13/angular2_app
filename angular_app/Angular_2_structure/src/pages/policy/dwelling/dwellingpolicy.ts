...
@Component({
    selector: 'page-dwelling-policy',
    templateUrl: 'dwellingpolicy.html'
})
export class DwellingPolicyPage implements OnInit {

    dwellingPolicyData: any;
    index: number;
    customerName: any;
    ...
    ngOnInit() {
        this.dwellingPolicyData = this.navParams.get('param1');
        this.index = this.navParams.get('param2');
        this.customerName = this.navParams.get('param3');
        this.dwellingPolicyData.csppolicyNickName = "DWELLING FIRE";
    }

    policyDocumentPage() {
        this.navCtrl.push(PolicyDocumentPage, {
            param1: this.dwellingPolicyData,
            param2: this.index,
   ...
   myBillsPage() {
        let text: string;
               this.paymentusService.getPolicy(this.dwellingPolicyData)
                    .map((policyRequest) => {
